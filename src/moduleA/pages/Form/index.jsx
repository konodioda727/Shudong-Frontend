...
import { AtInput,AtList,AtListItem,AtButton,AtModal, AtModalHeader, AtModalContent, AtModalAction } from 'taro-ui'
...
//订阅消息
    ...
    return (
        ...
        <AtModal isOpened={ifopen} >
          ...
          <AtModalContent>
          &nbsp;&nbsp;本平台保护所有用户的个人隐私权。我们将按照本隐私权政策的规定收集、使用或披露您的个人信息。同时，我们会通过本政策向您说明，我们如何为您提供访问、 更新、控制和保护您信息的服务。您使用或继续使用我们的服务，即意味着同意我们按照本《用户协议和隐私政策》收集、使用、储存和分享您的相关信息。如对本政策或相关事宜有任何问题，请通过微信或邮箱与我们联系。
          </AtModalContent>
          <AtModalContent>&nbsp;&nbsp;树洞救援团, 以帮助抑郁症患者缓解自杀情绪为责任, 以拯救生命为最高伦理原则. 如果您愿意成为救援团的志愿者, 本页面收集的您的姓名、手机号、邮箱等个人信息皆只用于您的身份审核, 确保不会泄漏您的个人信息. </AtModalContent>
          <AtModalAction> <Button onClick={handleCancle}>不同意</Button> <Button onClick={handleConfirm}>我已了解且同意该政策</Button> </AtModalAction>
        </AtModal>
       {/*  {Taro.getStorageSync('submit') && status!=3 && Taro.getStorageSync('submit') && <View className='hover' onClick={()=>{
        ...
        ></View>} */}
        <View className='Form'>
        <Text style={{fontSize:25,fontFamily:'sans-serif',marginBottom:30,marginLeft:20,color:'white'}}>申请表</Text>
...
{/*地址选择器 */}
          <View className='Form_i' onClick={chooseAddress} >
            ...
            <View className='Form_i'>
            <AtButton type='primary' onClick={register} size='normal'>提交</AtButton>
          </View>{/* } */}

        </View>
{/* 地址选择器 */}
          {showPicker && <AddressPicker city={true} address={address} onPick={onPickerChange} onAConfirm={onAConfirm}></AddressPicker>}
         {/*  {Taro.getStorageSync('submit')&&(
