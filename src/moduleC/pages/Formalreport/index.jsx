...
const Report=()=>{
    ...
    useReady(()=>{
        ...
        Fetch(
            ...
            res=>setFrequency(res.data.rescue_frequency)
        )
    })
    return(
        ...
        {pass?
         <View className='Grade'>
            <View className='box'>
                ...
                <View className='row1'>在你加入救助以来，你一共参与救援了<Text style={{fontWeight:400,fontSize:30,color:'#76A9FF'}}>{frequency}</Text>次</View>
                <View className='row1'>救援时长累计<Text style={{fontWeight:400,fontSize:30,color:'#76A9FF'}}>{duration}</Text></View>
                <View className='row1'>恭喜你正式成为树洞救援团的一员！期待未来和你一起救助更多的人！</View>
            </View>
        </View>:
        <View className='img'>
