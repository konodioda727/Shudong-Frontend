   ...
   function handleConfirm(){
    ...
    return(
        ...
        <View className='Modal' style={{display:show?'block':'none'}} id='modal'>
            ...
            <View className='show'>
                <View className='top'>
                    <Input placeholder='输入课程名称' value={Videoname} onInput={(e)=>handleInput(e)}></Input>
                </View>
                <View className='flex row'>
                    {/* <Button onClick={getInfo}>获取个人信息</Button> */}
                    <Button className='button' onClick={handleConfirm}>确定</Button>
    ...
    return(
        ...
        <View className='Modal' style={{display:show?'block':'none'}} id='modal'>
            ...
            <View className='show'>
                ...
                <View className='flex row'>
                    {/* <Button onClick={getInfo}>获取个人信息</Button> */}
                    <Button className='button' onClick={ondeleteVideo}>确定</Button>
                    <Button className='button' onClick={handleShow}>取消</Button>
                </View>
            </View>
        </View>
...
const VList = ()=>{
    ...
    return(
        <>
        <Modal changeShow1={changeShow1} modalShow1={modalShow1} videoID={videoID} getVideo={getVideo} />
        <ModalInput changeShow2={changeShow2} modalShow2={modalShow2} uploadVideo={uploadVideo} />
                ...
                video?video.map((item)=>{
                    return(
                        <View className='v-box' key={item.id} >
                            <View className='cover' onClick={()=>toPlay(item.url,item.file_name,item.id)}><Image src={cover} /></View>
                            <View className='v-name' onClick={()=>toPlay(item.url,item.file_name,item.id)}>{item.file_name}</View>
                            {Taro.getStorageSync('role')>=42?
                                <View className='v-delete' onClick={()=>deleteVideo(item.id)}>
                                    <Image src={del} />
                                </View>:''
                            }
                        </View>
